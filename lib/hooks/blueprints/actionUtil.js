...
module.exports = {
  ...
  populateEach: function ( query, req ) {
    ...
    var populate = req.param('populate') || _options.populate || false;
    var aliasFilter = !_.isBoolean(populate) ? populate : undefined;
    var shouldPopulate = _.isBoolean(populate) ? populate : false;
  ...
  parseCriteria: function ( req ) {
    ...
    req.options.criteria.blacklist = req.options.criteria.blacklist || ['limit', 'skip', 'sort', 'populate', 'fields'];
  ...
  parseValues: function (req) {
    ...
    req.options.values.whitelist = req.options.values.whitelist;
    req.options.values.blacklist = req.options.values.blacklist;

    // Validate whitelist to provide a more helpful error msg.
    var whitelist = req.options.values.whitelist;
    if (whitelist && !_.isArray(whitelist)) {
      throw new Error('Invalid `req.options.values.whitelist`. Should be an array of strings (parameter names.)');
    }
    ...
    // Pick built-in runtime config (like query modifiers)
    _.isEmpty(whitelist) || (values = _.pick(values, whitelist));
  ...
  parseFields: function (req) {
    return req.params.all().fields || req.options.fields || {};
  },

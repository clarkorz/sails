    ...
    return {
      ...
      */
      ...
      subscribe: function (req, records, contexts) {
        ...
        // todo: consider using the following instead (to lessen potential confusion w/ http requests)
        // (however it would need to be applied to all occurances of `req` in methods in this hook)
        // if (!socket.manager || !(req.socket && req.socket.manager)) {
        //   console.log('-----',req,'\n\n---------\n\n');
        //   sails.log.verbose('`Model.subscribe()` called by a non-socket request. Only requests originating from a connected socket may be subscribed. Ignoring...');
        //   return;
        // }
        ...
        if (options.previous && !options.noReverse) {
          ...
          _.each(changes, function(val, key) {
            ...
            if (referencedModel) {
              ...
              var reverseAssociation = _.find(ReferencedModel.associations, {collection: this.identity, via: key}) //reverse association is a to-Many with via
                                    || _.find(ReferencedModel.associations, {model: this.identity}); //reverse association is a to-One
                                    // TODO: what if via is absent? see https://github.com/balderdashy/sails/issues/1727
      ...
      publishDestroy: function (id, req, options) {
        ...
        if (options.previous) {
          ...
          _.each(this.associations, function(association) {
            ...
            var ReferencedModel, reverseAssociation;
            ...
            if (association.type == 'model' && [association.alias] && previous[association.alias]) {
              ...
              // Get the inverse association definition, if any
              reverseAssociation = _.find(ReferencedModel.associations, {collection: this.identity, via: association.alias}) || _.find(ReferencedModel.associations, {model: this.identity});
      ...
      watch: function ( req ) {
        ...
        // (however it would need to be applied to all occurances of `req` in methods in this hook)
        // if (!socket.handshake) {
        //   sails.log.verbose('`Model.watch()` called by a non-socket request. Only requests originating from a connected socket may be subscribed to a Model class. Ignoring...');
        //   return;
        // }
